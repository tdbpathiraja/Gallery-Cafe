...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES["new_image"]["size"] > 0) {
        ...
        if($check !== false) {
            ...
            if(in_array($fileType, $allowTypes)){
                ...
                if(move_uploaded_file($_FILES["new_image"]["tmp_name"], $targetFilePath)){
                    ...
                    $query = "UPDATE desserts SET name = ?, category = ?, price = ?, description = ?, ingredients = ?, time = ?, persons = ?, cuisines = ?, image_url = ? WHERE id = ?";
        ...
        $query = "UPDATE desserts SET name = ?, category = ?, price = ?, description = ?, ingredients = ?, time = ?, persons = ?, cuisines = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
    ...
    header('Location: ../../../../../../control-dashboards/admin/dashboard/desserts.php');
    exit();
}
?>
