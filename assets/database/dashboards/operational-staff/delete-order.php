...
if (isset($_POST['preorder_id'])) {
    ...
    $stmt = $conn->prepare(
        "SELECT u.email, u.username 
         FROM users u 
         JOIN pre_orders p ON u.username = p.username 
         WHERE p.preorder_id = ?"
    );
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            // Recipients
            $mail->setFrom('tdbpathiraja@gmail.com', 'Gallery Cafe');
            $mail->addAddress($userEmail);

            // Content
            $mail->isHTML(true);
            $mail->Subject = 'Order Deletion Notification';
            $mail->Body = "
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                <div class='container'>
                    ...
                    <div class='content'>
                        <p>Dear $username,</p>
                        <p>Your pre-order with ID <strong>$preorderId</strong> has been successfully deleted from our system.</p>
            ...
            $stmt = $conn->prepare("DELETE FROM pre_orders WHERE preorder_id = ?");
            $stmt->bind_param("s", $preorderId);
            if ($stmt->execute()) {
                echo 'success';
            } else {
