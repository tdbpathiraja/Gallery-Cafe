...
if ($conn) {
    // Query to select only dinner items
    $sql_menu = "SELECT * FROM MenuItems WHERE time = 'Dinner'";
    $result_menu = $conn->query($sql_menu);

    // Query to select only dinner beverages
    $sql_beverages = "SELECT * FROM Beverages WHERE time = 'Dinner'";
    $result_beverages = $conn->query($sql_beverages);

    // Query to select only dinner desserts
    $sql_desserts = "SELECT * FROM Desserts WHERE time = 'Dinner'";
    $result_desserts = $conn->query($sql_desserts);

    // Query to select only dinner side dishes
    $sql_sidedishes = "SELECT * FROM Sidedishes WHERE time = 'Dinner'";
    $result_sidedishes = $conn->query($sql_sidedishes);

    // Query to select only dinner Special Meals
    $sql_specialmeals = "SELECT * FROM SpecialMeals WHERE time = 'Dinner'";
