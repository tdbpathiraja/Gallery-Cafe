...
if ($conn) {
    ...
    $sql_menu = "SELECT * FROM MenuItems WHERE time = 'Tea Time'";
    $result_menu = $conn->query($sql_menu);

    // Query to select only lunch beverages
    $sql_beverages = "SELECT * FROM Beverages WHERE time = 'Tea Time'";
    $result_beverages = $conn->query($sql_beverages);

    // Query to select only lunch desserts
    $sql_desserts = "SELECT * FROM Desserts WHERE time = 'Tea Time'";
    $result_desserts = $conn->query($sql_desserts);

    // Query to select only lunch side dishes
    $sql_sidedishes = "SELECT * FROM Sidedishes WHERE time = 'Tea Time'";
    $result_sidedishes = $conn->query($sql_sidedishes);

    // Query to select only Tea Time Special Meals
    $sql_specialmeals = "SELECT * FROM SpecialMeals WHERE time = 'Tea Time'";
